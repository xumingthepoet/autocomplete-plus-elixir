...
class AutocompleteView extends SimpleSelectListView
  ...
  initialize: (@editorView) ->
    ...
    @registerProvider new ElixirProvider(@editorView)
    @registerProvider new SnippetsProvider(@editorView)
  ...
  viewForItem: ({show, label, renderLabelAsHtml, className}) ->
    item = $$ ->
      @li =>
        @span show, class: "word"
        if label?
  ...
  runAutocompletion: =>
    ...
    #@postRunAutoCompletion(suggestions)
    [head, tail...] = @providers.slice().reverse()
    head.asyncBuildSuggestions([], tail, (s) => @postRunAutoCompletion(s))

  # Private:
  # postRunAutoCompletion
  postRunAutoCompletion: (suggestions) =>
