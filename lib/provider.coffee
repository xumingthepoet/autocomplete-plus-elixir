...
class Provider
  ...
  postAsyncBuildSuggestions: (newSuggestions, preSuggestions, restProvders, finalCallback) ->
    if @exclusive
      finalCallback(newSuggestions)
    else
      if newSuggestions? and newSuggestions.length > 0
        preSuggestions = preSuggestions.concat newSuggestions
      if restProvders.length == 0
        finalCallback(preSuggestions)
      else
        [head, tail...] = restProvders
        head.asyncBuildSuggestions(preSuggestions, tail, finalCallback)
