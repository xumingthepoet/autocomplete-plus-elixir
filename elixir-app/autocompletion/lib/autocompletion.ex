defmodule :autocompletion do
  ...
  defp split_last_dot prefix do
    {a, b} = :lists.splitwith fn e -> e != ?. end, :lists.reverse prefix
    {:lists.reverse(b), :lists.reverse(a)}
  end

  defp getCompletion prefix, beforeDot, afterD do
    case expand :lists.reverse prefix do
      {:no, _, _} -> <<"">>
      {:yes, [], r} ->
        r = List.foldr r, [],
            fn e, acc ->
              show = e
              word = beforeDot ++ show
              word2 = tl(:lists.reverse(word))
              if hd(word2) == ?/ do
                word = :lists.reverse(tl(word2))
              end
              [?,|[show|[?;|[word | acc]]]]
            end
        :erlang.list_to_binary tl r
      {:yes, ?., []} -> <<"">>
      {:yes, postfix, []} ->
        if List.last(postfix) == ?. do
          show = :lists.reverse(tl(:lists.reverse(postfix)))
          word = prefix ++ show
          show = afterD ++ show
          :erlang.list_to_binary :lists.append(show, [?; | word])
        else
          prefix = prefix ++ postfix
          getCompletion(prefix, beforeDot, afterD)
        end
    end
  end
